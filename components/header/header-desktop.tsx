...
export default function HeaderDesktopFull({ changeAt }: { changeAt: number }) {
  ...
  const handleSelectChange = (lang: string) => {
    setLanguage(lang); // Sử dụng setLanguage từ context
  };
  ...
  return (
    <header
      className={`w-full ${
        isScrolled || !isDesktop ? "canvas-bg-1 shadow-md " : "bg-transparent"
      } transition-all duration-300 ease-in-out fixed h-[72px] top-0 flex items-center justify-center ${
        !isMobile ? "px-10" : "px-5"
      } py-4`}
      style={{ zIndex: 1000 }}
    >
      <div className="flex items-center justify-between flex-[1_0_0] transition-all duration-300 ease-in-out max-w-[1200px]">
        <Image
          src={isScrolled || !isDesktop ? logoFullDark : logoFull}
          alt="Logo"
          width={117}
          height={28}
          className="w-[117px] h-[28px]"
        />
        {isDesktop && (
        ...
        <div className="flex flex-row gap-2 items-center justify-items-start transition duration-300 ease-in-out opacity-100 text-sm z-[1001]">
          {!isMobile && (
            <>
              ...
              <a href="https://wealthfarming.app/" target="_blank">
                <Button
                  variant="outline"
                  className="!bg-[var(--color-primary)] hover:brightness-[1.1] border-none button rounded-none w-[148px] h-[40px] transition-transform duration-300 ease-in-out "
                >
          ...
          {!isScrolled && isDesktop && (
            <Select onValueChange={handleSelectChange} defaultValue={language}>
              <SelectTrigger className="!h-[43px] flex flex-row items-center justify-between bg-background !rounded-[4px] w-[200px]">
                ...
                <SelectValue placeholder={language === "en" ? "vi" : "en"} />
              </SelectTrigger>
          ...
          {!isDesktop && (
            <div
              className="flex p-0 m-0 items-center justify-center border rounded-[6px] w-[40px] h-[40px]"
      ...
      {isMobile && isMenuOpen && (
        <div className="absolute top-[72px] left-0 w-full bg-white p-4 flex flex-col items-center border-t-2 border-b-2 border-gray-200">
          <Link href="/" className="py-2 text-[11px]">
            {t("header_introduct")}
          </Link>
          <Link href="/product" className="py-2 text-[11px]">
            {t("header_product")}
          </Link>
          <Link href="/ecosystem" className="py-2 text-[11px]">
            {t("header_eco")}
          </Link>
          <Link href="/investment-bank" className="py-2 text-[11px]">
            {t("header_investBank")}
          </Link>
          ...
          <div className="flex w-full justify-between gap-4 mb-2">
            ...
            <a href="https://www.facebook.com/people/Wealth-Farming/61574682066111/" target="_blank" className="w-[48%]">
              <Button
                variant="outline"
                className="w-full !bg-background hover:brightness-[0 Folio95] button border-none rounded-none h-[40px] flex items-center justify-center gap-2 transition-transform duration-300 ease-in-out text-[11px]"
              >
                <ChatCenteredDots size={24} className="min-w-[20px] min-h-[20px]" />
                <p>{t("free_support")}</p>
              </Button>
